...
fn default_rendering_uses_the_formatted_string(cx: &mut TestAppContext) {
    ...
    assert_eq!(
        observations.last_root_state().formatted.as_deref(),
        Some("50%")
    );
}

#[gpui::test]
fn default_rendering_has_no_text_when_indeterminate(cx: &mut TestAppContext) {
    let window = open_progress(cx, ProgressTestConfig::default());

    let observations = read_observations(cx, window);
    assert_eq!(observations.last_root_state().formatted, None);
}
...
fn display_override_receives_formatted_and_raw_value(cx: &mut TestAppContext) {
    ...
    assert_eq!(call, (Some(String::from("50%")), Some(50.0)));
}

#[gpui::test]
fn display_override_receives_none_when_indeterminate(cx: &mut TestAppContext) {
    let window = open_progress(
        cx,
        ProgressTestConfig {
            value: None,

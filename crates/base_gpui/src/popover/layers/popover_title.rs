...
use crate::popover::{
    ...
    PopoverContext, PopoverTitleStyleState,
};

#[derive(IntoElement)]
pub struct PopoverTitle<P: Clone + 'static = ()> {
    id: ElementId,
    base: Div,
    children: Vec<AnyElement>,
    context: Option<PopoverContext<P>>,
    style_with_state: Option<Rc<dyn Fn(PopoverTitleStyleState, Div) -> Div + 'static>>,
}

impl<P: Clone + 'static> Default for PopoverTitle<P> {
    fn default() -> Self {
        Self {
            id: ElementId::from("popover-title"),
            base: div(),
            children: Vec::new(),
            context: None,
...
impl<P: Clone + 'static> ParentElement for PopoverTitle<P> {
    fn extend(&mut self, elements: impl IntoIterator<Item = AnyElement>) {
        self.children.extend(elements);
    }
}

impl<P: Clone + 'static> Styled for PopoverTitle<P> {
    fn style(&mut self) -> &mut StyleRefinement {
        self.base.style()
    }
}

impl<P: Clone + 'static> RenderOnce for PopoverTitle<P> {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        let state = self
            .context
            .as_ref()
            .map(|context| context.read(cx, |runtime, _| runtime.title_state()))
            .unwrap_or(PopoverTitleStyleState);
...
impl<P: Clone + 'static> PopoverChildNode<P> for PopoverTitle<P> {
...
impl<P: Clone + 'static> PopoverTitle<P> {
    pub fn new() -> Self {
        Self::default()
    }

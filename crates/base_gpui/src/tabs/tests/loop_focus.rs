...
use super::support::{
    ACCOUNT, OVERVIEW, TabsTestConfig, open_tabs, read_observations, simulate_keys, update_config,
};
...
fn loop_focus_wraps_when_enabled_and_clamps_when_disabled(cx: &mut TestAppContext) {
    ...
    assert_eq!(read_observations(cx, window).highlighted_value(), Some(ACCOUNT));
    ...
    assert_eq!(read_observations(cx, window).highlighted_value(), Some(OVERVIEW));
}

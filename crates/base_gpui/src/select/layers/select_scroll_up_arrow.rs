...
#[derive(IntoElement)]
pub struct SelectScrollUpArrow<T: Clone + Eq + 'static> {
    base: Div,
...
impl<T: Clone + Eq + 'static> Default for SelectScrollUpArrow<T> {
    fn default() -> Self {
        Self {
            base: div(),
            children: Vec::new(),
...
impl<T: Clone + Eq + 'static> ParentElement for SelectScrollUpArrow<T> {
    fn extend(&mut self, elements: impl IntoIterator<Item = AnyElement>) {
        self.children.extend(elements);
    }
}

impl<T: Clone + Eq + 'static> Styled for SelectScrollUpArrow<T> {
    fn style(&mut self) -> &mut StyleRefinement {
        self.base.style()
    }
}

impl<T: Clone + Eq + 'static> RenderOnce for SelectScrollUpArrow<T> {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        base.on_mouse_move(move |_event, window, cx| {
            if let Some(context) = scroll_context.as_ref() {
                context.scroll_toward(SelectScrollArrowDirection::Up, window, cx);
            }
        })
...
impl<T: Clone + Eq + 'static> SelectChildNode<T> for SelectScrollUpArrow<T> {
...
impl<T: Clone + Eq + 'static> SelectScrollUpArrow<T> {
    pub fn new() -> Self {
        Self::default()
    }

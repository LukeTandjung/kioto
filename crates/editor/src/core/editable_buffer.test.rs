...
fn clamps_past_end_of_text() {
    let clamped = clamp_range("ab", Range::new(Position(1), Position(99)));
...
fn clamps_mid_character_offsets_to_previous_boundary() {
    // 💝 occupies bytes 1..5.
    let clamped = clamp_range("a💝b", Range::new(Position(2), Position(5)));
...
fn collapses_inverted_clamps_to_empty() {
    ...
    let clamped = clamp_range("a💝b", Range::new(Position(2), Position(3)));

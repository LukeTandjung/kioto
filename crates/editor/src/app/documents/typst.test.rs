...
fn edits_keep_text_and_syntax_tree_in_sync() {
    ...
    CoreActions::replace_chars(&mut document, Range::new(Position(2), Position(9)), "Title");
...
fn clamps_out_of_range_and_mid_character_edits() {
    ...
    CoreActions::insert_chars(&mut document, Position(99), "c");
    assert_eq!(document.text(), "a💝bc");
    CoreActions::delete_chars(&mut document, Range::new(Position(2), Position(5)));
...
fn heading_hides_markers_and_carries_level() {
    ...
    assert_eq!(blocks[0].offset_map.display_to_source(0), 3);
    assert_eq!(blocks[0].offset_map.source_to_display(3), 0);
}
...
fn cursor_inside_a_block_reveals_raw_markup() {
    ...
    assert_eq!(output.blocks[0].offset_map.source_to_display(3), 3);
...
fn preview_survives_incremental_edits() {
    ...
    CoreActions::insert_chars(&mut document, Position(7), "!");

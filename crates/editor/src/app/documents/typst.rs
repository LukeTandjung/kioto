...
impl EditableBuffer for TypstDocument {
    ...
    fn replace(&mut self, range: Range, new_text: &str) {
        let clamped = clamp_range(self.source.text(), range);
...
impl PreviewRenderer for TypstDocument {
    fn render_preview(&self, cursor_positions: &[Position]) -> PreviewOutput {
        ...
        for block in &mut blocks {
            ...
            if contains_cursor {
                *block = raw_block(self.source.text(), block.source_range);
            }
        }
...
fn raw_block(text: &str, source_range: Range) -> PreviewBlock {
    let range: std::ops::Range<usize> = source_range.into();
    PreviewBlock {
        source_range,
        kind: BlockKind::Raw,
...
impl<'a> BlockSink<'a> {
    ...
    fn push_verbatim(&mut self, node: &SyntaxNode, kind: SpanKind) {
        let range: std::ops::Range<usize> = self.document.node_range(node).into();
        if range.is_empty() {
            return;
        }

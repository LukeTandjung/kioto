...
- [x] Add a `slider` module and export it from `crates/base_gpui/src/lib.rs`.
- [x] Register slider key bindings from `base_gpui::init(cx)` via `slider::init(cx)`.
- [x] Add public `SliderRoot`, `SliderControl`, `SliderTrack`,
      `SliderIndicator`, `SliderThumb`, `SliderValue`, and `SliderLabel`
      layer types.
- [x] Add `SliderValues` (`Single(f64)` / `Range(Vec<f64>)`) as the public
      value type.
- [x] Add `SliderOrientation` (`Horizontal` default, `Vertical`).
- [x] Add `SliderThumbCollisionBehavior` (`Push` default, `Swap`, `None`).
- [x] Add `SliderThumbAlignment` (`Center` default, `Edge`).
- [x] Support root `.id(...)` as the stable keyed identity.
- [x] Support `.name(...)` metadata.
- [x] Support uncontrolled `.default_value(SliderValues)`; absent default
      initializes to `Single(min)`.
- [x] Support controlled `.value(SliderValues)`.
- [x] Support `.on_value_change(...)` receiving the proposed `SliderValues`
      and `&mut SliderValueChangeDetails` (cancelable).
- [x] Support `.on_value_committed(...)` receiving the committed
      `SliderValues` and non-cancelable commit details.
- [x] Support `.min(f64)` (default `0.0`) and `.max(f64)` (default `100.0`).
- [x] Support `.step(f64)`, defaulting to `1.0`, with `min` as the step-grid
      origin.
- [x] Support `.large_step(f64)`, defaulting to `10.0`.
- [x] Support `.min_steps_between_values(f64-count or usize)`, defaulting to
      `0`, expressed in steps (distance = `step * min_steps_between_values`).
- [x] Support `.orientation(...)`, defaulting to horizontal.
- [x] Support `.thumb_collision_behavior(...)`, defaulting to `Push`.
- [x] Support `.thumb_alignment(...)`, defaulting to `Center`.
- [x] Support `.format(...)` as a Rust closure formatting a single `f64` for
      `SliderValue` display (no `Intl` options).
- [x] Support root `.disabled(bool)`, defaulting to `false`.
- [x] Support per-thumb `.disabled(bool)` on `SliderThumb`, defaulting to
      `false`, merged with root/Field disabled.
- [x] `SliderValue` supports a custom display closure over
      (formatted values, raw values); default display joins formatted values
      with `" – "`.
- [x] Expose ergonomic barrel exports from `slider/mod.rs` (barrel only —
      no items defined in `mod.rs`).
- [x] Do not expose CSS class, web style, CSS variable, or data-attribute
      APIs.
...
- [x] `ast-grep scan` passes (no `pub(...)` scoped visibility).
- [x] Debug builds warn (log or debug assert) when `min >= max`, matching
      Base UI's dev warning; release behavior stays defined (no panic).

### Architecture / internal primitives

- [x] `SliderRuntime` is the single deep module owning: normalized sorted
...
- [x] `SliderProps` holds stable public configuration and callbacks only —
      no runtime state.
- [x] `SliderContext` stays thin: `read` / `update` / the value-changing
...
- [x] The runtime never calls user callbacks; value-change commands return
      outcomes and the context/root fire `on_value_change` /
      `on_value_committed`.
- [x] Typed `SliderChild` enum for root children (`Control`, `Track` nesting
...
- [x] `child_wiring.rs` is the only place that assigns thumb indices (in
      declaration order) and attaches context; no index bookkeeping in parts.
- [x] Root render is the single non-event mutation site: wire children,
...
- [x] Control and thumb bounds are measured via prepaint
...
- [x] Position→value conversion, closest-thumb selection, collision
      resolution, step rounding, and clamping all live in the runtime /
      `math.rs`, not in layers.
- [x] Pure value math in `math.rs` has no GPUI types except `Pixels`/`Bounds`
      where geometry is inherent, and is unit-testable without a window.
- [x] Step/precision helpers are shared with or deliberately aligned to
      `number_field/number.rs`; no silently divergent duplicate of the same
      decimal-precision rule.
- [x] No new shared generic primitives; slider-specific helpers stay under
      `slider/`.
...
- [x] Controlled root reflects `.value(...)` across renders; interactions
      fire `on_value_change` without mutating internal state as source of
      truth.
- [x] Controlled precedence: if `.value(...)` is supplied the root is
      controlled; otherwise uncontrolled.
- [x] Range values are normalized to sorted ascending order for rendering
      and math even if provided unsorted.
- [x] Single values outside `[min, max]` are clamped for rendering/math
      (Base UI clamps the derived value; the caller's controlled prop is not
      mutated).
- [x] `on_value_change` fires at most once per applied change; a proposed
      value equal to the current value (scalar equality / element-wise array
      equality) is a no-op that fires nothing.
- [x] NaN proposals are rejected without firing callbacks.
- [x] `details.cancel()` inside `on_value_change` prevents the internal
      update (uncontrolled) and suppresses the eventual commit callback for
      that interaction, matching `changeDetails.isCanceled`.
- [x] A canceled swap does not leak the swapped thumb index into subsequent
      drag moves (pressed index only updates when the change is applied).
- [x] `on_value_committed` fires only if the value actually changed during
      the interaction and the change was not canceled.
- [x] Value shape is preserved through callbacks: `Single` in → `f64`-shaped
      `Single` out; `Range` in → `Range` out with the same length.
- [x] Re-rendering with changed unrelated props does not reset uncontrolled
      values, focus, or drag state unless the keyed `.id(...)` changes.

### Value / step / clamp math

- [x] Port `roundValueToStep`: nearest step multiple with `min` as origin,
      rounded to `max(decimal_precision(step), decimal_precision(min))`
      decimal places.
- [x] Port `getDecimalPrecision`, including the small-magnitude
      (`|x| < 1`, exponential notation) branch.
- [x] Port `valueToPercent` (`(value - min) * 100 / (max - min)`) or its
      fraction equivalent used for `relative()` lengths.
- [x] Port `getSliderValue` neighbor clamping: keyboard/programmatic
...
- [x] Port `validateMinimumDistance`: a proposed range is rejected when any
      adjacent pair is closer than `step * min_steps_between_values`.
- [x] Keyboard stepping first rounds the current value to the step grid,
      ...
      then clamps.
- [x] Floating-point noise is cleaned for common cases (`0.1 + 0.2`,
      `toFixed(12)`-style cleanup in collision/push results).
- [x] Pointer position→value: fraction of measured control main-axis size,
...
- [x] All math helpers have focused unit tests translated from
...
- [x] Left-button press only; presses on a disabled slider or a disabled
      thumb do nothing (disabled thumb press also resets pressed-thumb
      state).
- [x] Press on the control (not a thumb): the nearest enabled thumb by
...
- [x] Press on a thumb: pressed index and pointer-to-thumb-center offset are
      recorded; no value change is applied until movement; the thumb receives
      focus.
- [x] Pressing a thumb whose value equals `max` while stacked on other
...
- [x] Drag-start values are captured as the baseline for push/swap
      restoration for the duration of the interaction.
- [x] Moves convert position→value with the recorded center offset so the
      value does not jump on the first move.
- [x] `dragging` style state turns true only after more than 2 move events
      for a thumb press (INTENTIONAL_DRAG threshold), immediate for a track
      press.
- [x] Moves that violate minimum distance are ignored (no change, no
      callback).
- [x] Drag changes fire `on_value_change` with reason `Drag` and the active
      thumb index each time the resolved value differs.
- [x] The drag continues receiving moves when the pointer leaves the control
      bounds (GPUI capture-equivalent handlers replace DOM pointer capture +
      document listeners).
- [x] If button state is observed released without an up event, the drag
      ends as if released (Base UI `buttons === 0` guard) — or document why
      GPUI cannot observe this and that `on_mouse_up_out` covers it.
- [x] Release: clears active/dragging/pressed state and fires
      `on_value_committed` (with the last change reason) only if a change
      was applied during the interaction.
- [x] Disabling the slider mid-interaction stops listening and clears
      transient drag state; active index resets to none while disabled.

### Keyboard step behavior

- [x] `slider/actions.rs` defines actions bound in a slider key context set
...
- [x] ArrowUp increments and ArrowDown decrements by `step`, regardless of
      orientation.
- [x] ArrowRight/ArrowLeft increment/decrement by `step` in LTR and are
      flipped in RTL (via `utils/direction.rs`), for both orientations.
- [x] Shift+Arrow uses `large_step` in place of `step`, same direction rules.
- [x] PageUp/PageDown increment/decrement by `large_step`.
- [x] Home sets the thumb to `min`; End sets it to `max`.
- [x] In range mode, Home/End clamp to the neighbor boundary:
...
- [x] Keyboard changes go through the neighbor-clamped path
      (`getSliderValue` port) — keyboard never pushes or swaps thumbs.
- [x] Keyboard changes that violate minimum distance are ignored.
- [x] Applied keyboard changes fire `on_value_change` (reason `Keyboard`),
      mark the field touched, and commit immediately via
      `on_value_committed` (reason `Keyboard`).
- [x] Keyboard on a disabled slider or disabled thumb does nothing.
- [x] Each thumb is a tab stop via its `FocusHandle`; focusing a thumb sets
      it active; blurring clears active and marks touched.

### Thumb collision behaviors

- [x] Port `resolveThumbCollision` and `getPushedThumbValues` as pure
      functions in `math.rs`, translating Base UI's unit tests
      (`resolveThumbCollision.test.ts`, `getPushedThumbValues.test.ts`).
- [x] Single-value sliders bypass collision resolution entirely.
- [x] `Push` (default): dragging a thumb past a neighbor pushes the neighbor
...
- [x] `Swap`: dragging past a neighbor (epsilon `1e-7`) swaps the pressed
...
- [x] `None`: the pressed thumb clamps to
      `[prev + min_distance, next - min_distance]`; excess movement is
      ignored.
- [x] Collision results are precision-cleaned (`toFixed(12)` equivalent).
- [x] Collision behavior applies only to pointer interactions; keyboard uses
      neighbor clamping regardless of the configured behavior.

### Field / Form integration

- [x] Slider inside `FieldRoot` merges Field disabled state
      (`field_disabled || slider_disabled`).
- [x] Slider inside a disabled `FieldItem` is disabled.
- [x] `SliderRoot` registers one `FieldControlRegistration` with a stable
      key.
- [x] Registration name uses Field name when present, else the slider's
      `.name(...)` (Base UI: `fieldName ?? nameProp`).
- [x] Registration includes a value representation compatible with existing
      `FieldValue` variants (formatted text or `Present`); add a numeric
      variant only if it stays shallow for existing controls.
- [x] Registration includes merged disabled, focused state, and the first
      thumb's focus handle so `FieldLabel` click focuses the slider.
- [x] A slider always registers as filled (its value always exists).
- [x] Field becomes dirty when values differ from the initially registered
      values (element-wise for ranges).
- [x] Field becomes focused while any thumb is focused; touched when a thumb
...
- [ ] `FieldValidationMode::OnChange` validates on applied value changes;
      ...
      *(Skipped: validation timing is delegated to the shared Field registration pass, as in Number Field; not explicitly verified for slider.)*
- [ ] Applied value changes clear matching Form errors (Base UI
      `clearErrors(name)`).
      *(Skipped: relies on the shared Field/Form registration machinery; no slider-specific clear-errors call was added or verified.)*
- [x] `SliderLabel` click focuses the thumb when the slider has exactly one
      thumb; with multiple thumbs it is a no-op (Base UI only falls back when
      exactly one input exists).

### Orientation / RTL behavior

- [x] Horizontal sliders map the main axis to x; vertical sliders map to y
      with values increasing upward (position measured from the bottom
      edge).
- [x] RTL horizontal sliders (via `DirectionProvider`) invert
      position→value mapping (measured from the right edge) and thumb /
      indicator placement.
- [x] RTL flips ArrowLeft/ArrowRight stepping; ArrowUp/ArrowDown and
      PageUp/PageDown are unaffected.
- [x] Vertical sliders keep drag, track-press, closest-thumb, and indicator
      math correct on the y axis.
- [x] Orientation is exposed in style state for all parts.

### Positioning / thumb alignment

- [x] Thumbs are absolutely positioned along the main axis using
...
- [x] `SliderIndicator` (center alignment): single → spans from the start
      edge to `percent(value)`; range → spans from `percent(first)` to
      `percent(last)`.
- [x] Active-thumb stacking: in range mode the active thumb renders above
      others and the last-used thumb above the rest (Base UI z-index 2/1);
      single mode raises the active thumb.
- [x] `SliderThumbAlignment::Edge` computes inset positions from measured
      ...
      frame.
- [x] Edge alignment also insets the pointer position→value conversion by
      half the thumb size on each end.
- [x] Before first measurement, edge-aligned thumbs/indicator expose a
      not-yet-positioned state (Base UI hides them) rather than flashing at
      a wrong position.

### Styling / state exposure

- [x] Add `SliderRootStyleState`, `SliderControlStyleState`,
...
- [x] Style states expose (where relevant): `values`, `min`, `max`, `step`,
...
- [x] `SliderThumbStyleState` additionally exposes the thumb's index, value,
      formatted value, focused/active flags, per-thumb disabled, and its
      resolved main-axis position fraction.
- [x] `SliderValueStyleState` exposes formatted values and raw values.
- [x] Every public layer supports `style_with_state(...)` receiving its
      typed style state.
- [x] Queries are part-shaped ("this thumb's state"), not runtime-internal
      ("the active index"), per the architecture doc.
- [x] Do not expose DOM data attributes or CSS variable names as styling
      API.

### Docs / demo

- [x] Update `crates/base_gpui/src/main.rs` (or an example) with a single
      slider and a two-thumb range slider demo, including a
      `FieldRoot` + `FieldLabel` + slider + `FieldError` composition.
- [x] Document the supported phase-1 prop subset and explicitly list dropped
...
- [x] `round_value_to_step` cases translated from `roundValueToStep.test.ts`
      (decimal steps, decimal min origin, small-magnitude precision).
- [x] `get_slider_value` neighbor-clamp cases from `getSliderValue.test.ts`.
- [x] `resolve_thumb_collision` push/swap/none cases from
      `resolveThumbCollision.test.ts` (including canceled-swap index rules,
      epsilon boundaries, min-distance spacing).
- [x] `get_pushed_thumb_values` cases from `getPushedThumbValues.test.ts`
      (push budgets at min/max, restoration toward initial values).
- [x] `validate_minimum_distance` accepts/rejects correctly for 2 and 3+
      thumbs.
- [x] Position→value conversion: horizontal LTR, horizontal RTL, vertical,
      center offset subtraction, and edge-alignment inset.
- [x] Uncontrolled default value (`Single(min)` when absent; provided
      default otherwise).
- [x] Controlled value is reflected; interaction fires `on_value_change`
      without self-mutation; unsorted controlled ranges render sorted.
- [x] `details.cancel()` prevents the change and the commit.
- [x] Equal-value proposal fires no callbacks.
- [x] Track press applies the closest-thumb value with reason `TrackPress`
...
- [ ] Thumb press applies no change until movement; the first move does not
      ...
      *(Partially: press-without-change is unit tested; the center-offset no-jump case lacks a windowed test.)*
- [x] Max-stacked thumb press re-targets to the first thumb of the stack.
- [ ] Dragging past a neighbor pushes (default), swaps with
      `Swap` (focus follows), and clamps with `None`.
      *(Partially: push/swap/none covered by math/runtime unit tests; windowed focus-follows-swap untested.)*
- [x] `dragging` style state becomes true only after the intentional-drag
      threshold for a thumb press.
- [x] Release fires `on_value_committed` exactly once, only when a change
      was applied.
- [x] Keyboard: Arrow/Shift+Arrow/PageUp/PageDown/Home/End step and clamp
...
- [x] Decimal-step keyboard stepping stays precision-clean
...
- [ ] Disabled slider ignores pointer and keyboard; a per-thumb disabled
      ...
      *(Partially: disabled slider tested; per-thumb closest-thumb skip implemented but untested.)*
- [ ] `min_steps_between_values` blocks drag and keyboard changes that
      violate spacing.
      *(Partially: keyboard blocking unit tested; drag blocking implemented via the same validation but untested.)*
- [x] Indicator geometry matches value percents for single and range, both
      orientations.
- [ ] Edge alignment positions thumbs/indicator from measured sizes and
      updates when sizes change.
      *(Skipped: edge alignment implemented but has no measurement-driven test.)*
- [ ] `SliderValue` renders the default joined formatted display and a
      custom display closure.
      *(Partially: default joined display tested; custom display closure untested.)*
- [ ] Field integration: label click focuses a single-thumb slider; filled
      ...
      *(Skipped: field integration implemented via registration but has no rendered tests.)*
- [x] Style states expose dragging/active/disabled/orientation/values for
      every part.
- [x] `cargo test -p base_gpui slider` passes.

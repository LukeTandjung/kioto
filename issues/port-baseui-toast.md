...
- [ ] `crates/base_gpui/src/toast/` exists with the expected flat component layout.
- [ ] `toast/mod.rs` and `toast/layers/mod.rs` are barrel-only files (module
      declarations, re-exports, test declarations only).
- [ ] `base_gpui::toast` is exported from `crates/base_gpui/src/lib.rs` and
      `base_gpui::init(cx)` calls `toast::init(cx)` for action/key registration.
- [ ] `ToastProvider<P>`, `ToastViewport<P>`, `ToastRoot<P>`, `ToastContent<P>`,
      `ToastTitle<P>`, `ToastDescription<P>`, `ToastClose<P>`, `ToastAction<P>`, and
      `ToastPortal<P>` exist and are publicly exported.
- [ ] `ToastManager<P>` and `create_toast_manager::<P>()` exist and are publicly
      exported from `toast/manager.rs`.
- [ ] A GPUI-native `useToastManager` analog exists for code with app access (e.g.
...
- [ ] `ToastId` is a typed id (not a raw string parameter type on every API); the
      runtime generates unique ids when the caller supplies none.
- [ ] Toast add/update options carry: optional `id`, `title`, `description`, typed
...
- [ ] Payload APIs use `P: Clone + 'static`; no `serde_json`-style dynamic values.
- [ ] Per-toast content rendering is a typed GPUI content-builder on the viewport (or
      equivalent explicit Rust API) receiving typed toast facts and returning a
      `ToastRoot<P>` subtree; no React-style render props.
- [ ] All public style payloads use `*StyleState` names from `style_state.rs`; no
      `*RenderState` / `render_state.rs`.
- [ ] No Toast code uses Rust scoped visibility syntax (`pub(...)`).
...
- [ ] `cargo clippy -p base_gpui --all-targets` passes or only reports pre-existing
      warnings.
- [ ] `crates/base_gpui/src/main.rs` gains gallery smoke scenarios: basic add/dismiss,
...
- [ ] `ToastRuntime<P>` (in `runtime.rs`) is the single deep module owning **all**
...
- [ ] `ToastRuntime<P>` exposes domain commands, one per thing-that-can-happen, e.g.:
...
- [ ] `ToastRuntime<P>` exposes part-shaped queries returning `Toast*StyleState`
...
- [ ] Every transition (limited recomputation, ending status, metadata/offset
      recalculation, timer rescheduling decisions) is computed inside the runtime,
      once; parts never diff shadow copies.
- [ ] Timer **decisions** live in the runtime; timer **execution** lives in layers:
...
- [ ] The runtime's time arithmetic (remaining-ms on pause, resume-with-remaining) is
      expressed against injected `Instant`s / durations passed into commands, so it is
      unit-testable without a window or real clock.
- [ ] `ToastContext<P>` remains a thin injection vehicle (`read`, `update`, and the
      value-changing command wrappers); it grows no queue/timer/measurement
      vocabulary of its own.
- [ ] `ToastProvider<P>` creates and owns the keyed runtime entity per provider
...
- [ ] `ToastManager<P>` is a late-bound handle (event-bus analog of
...
- [ ] If an app-global convenience accessor is provided (gpui-component
...
- [ ] Child routing uses typed enums before `AnyElement` erasure; toast-root children
...
- [ ] No new shared generic primitive is introduced under `utils/` for toast plumbing.
- [ ] No `runtime_control.rs`-style trait-boundary file is introduced.

### Queue, limit, and upsert behavior

- [ ] New toasts are prepended (newest-first queue order preserved exactly).
- [ ] Provider `timeout` defaults to 5000 ms; `limit` defaults to 3.
- [ ] `add` with no id generates a unique id and returns it.
- [ ] `add` with an existing id whose toast is **not** ending upserts in place: fields
      updated, auto-dismiss timer reset, update generation incremented, queue position
      unchanged, and the same id returned.
- [ ] `add` with an existing id whose toast **is** ending removes the ending toast
      without firing `on_remove`, then adds a fresh toast.
- [ ] `update(id, ...)` on a live toast applies partial updates and increments the
      update generation; `update` on an ending toast is ignored (prevents async
      promise updates from blocking a dismissal).
- [ ] `update` clears the timer when the toast should no longer have one (became
...
- [ ] Over-limit behavior: active (non-ending) toasts beyond `limit`, oldest first,
...
- [ ] `limited` flags are recomputed on every add/close/limit change; ending toasts
      are ignored by the limit count; a toast whose flag is unchanged is not treated
      as updated.
- [ ] Changing provider `limit`/`timeout` at render time syncs into the runtime and
      recomputes `limited` flags (Base UI `syncProviderProps` parity).
- [ ] `close(Some(id))` marks that toast `ending` (height treated as 0 for stacking)
      and cancels its timer; `close(None)` closes **all** toasts and clears all timers.
- [ ] `on_close` fires when a toast transitions to `ending` (once — not re-fired for
      already-ending toasts); `on_remove` fires when the toast is actually removed
      after its exit transition.
- [ ] When no active (non-ending) toasts remain, hovering/focused facts reset so a
      future toast starts from a collapsed viewport.
- [ ] Removal is deferred until the exit animation/transition completes; with no
      animation infrastructure active, removal completes immediately after `ending`.

### Timer pause/resume model

- [ ] A toast's dismiss duration is its own `timeout` if set, else the provider
      `timeout`.
- [ ] Duration `0` means sticky: no timer is ever scheduled.
- [ ] `toast_type = Loading` never schedules a timer regardless of duration.
- [ ] A toast added while the viewport is hovered, focus-visible-focused, or the
      window is unfocused starts with its timer **paused** (full duration remaining).
- [ ] `pause_timers` is idempotent and records each toast's remaining duration
      (`delay - elapsed`, clamped at 0).
- [ ] `resume_timers` is idempotent and restarts each timer with its remaining
      duration (full delay if never started).
- [ ] Viewport hover (mouse enter/move) pauses all timers and sets hovering; mouse
...
- [ ] After the last timer fires or is cleared, internal paused bookkeeping resets so
...
- [ ] Timer expiry closes the toast (transition to `ending`), not instant removal.
- [ ] Stale timer completions (generation mismatch after reset/pause/close) never
      mutate current state.

### Swipe-to-dismiss behavior

- [ ] `ToastRoot<P>::swipe_direction(...)` accepts one or more of `Up`, `Down`,
      `Left`, `Right`; default is `[Down, Right]`; an empty set disables swiping.
- [ ] Swipe is implemented with GPUI mouse handlers (down/move/up on the root plus
      window-level move/up capture while dragging); no DOM pointer capture is ported.
- [ ] Mouse-down on an interactive descendant (Close, Action, or other focusable
      control) does not begin a swipe.
- [ ] Drag displacement beyond a minimum threshold (Base UI: 1 px) marks a real swipe;
      when both axes are allowed, the dominant axis at that moment locks the drag
      direction.
- [ ] Displacement along a permitted direction moves the toast 1:1; displacement along
      non-permitted directions is damped (Base UI: `delta ** 0.5`).
- [ ] Release with displacement beyond the dismiss threshold (Base UI: 40 px) in a
      permitted direction closes the toast, recording the dismiss direction.
- [ ] Change-of-mind cancel: when only one direction on an axis is permitted, moving
...
- [ ] Release below the threshold (or a cancelled gesture) resets the toast offset to
      rest and clears the swipe direction.
- [ ] Swiping facts (`swiping`, `swipe_direction`, current x/y swipe movement) are
...
- [ ] While dragging, timers stay paused (the pointer is over the viewport; verify the
      ...
      viewport).
- [ ] Escape closes the toast whose subtree contains focus, via `actions.rs` key
      dispatch + `key_context` on the root (not raw key-down handlers).

### Stacking, measurement, and transition behavior

- [ ] Each toast's natural height is measured GPUI-natively (prepaint /
...
- [ ] Height changes recompute stacking metadata: per-toast `offset_y` is the
      cumulative height of preceding (newer) toasts in queue order; ending toasts
      contribute 0.
- [ ] `ToastRootStyleState` exposes: `transition_status` (`Starting`/`Ending`/steady),
...
- [ ] `ToastViewportStyleState` exposes `expanded` (hovering || focused) and
      `frontmost_height` (newest toast's measured height, for collapsed-stack sizing).
- [ ] `ToastContentStyleState` exposes `expanded` and `behind` (visible index > 0).
- [ ] `transition_status` starts at `Starting` on add and clears after the first
      height measurement (so enter animations can key off it), matching Base UI.
- [ ] `Ending` status is preserved until removal so exit animations can run; where
...
- [ ] Toasts render in the viewport via GPUI deferred/anchored rendering
...
- [ ] `promise(future, options)` adds a `Loading` toast immediately (no timer) and
...
- [ ] On success, the toast updates in place to the success options with
      `toast_type = Success` and gains a dismiss timer per the resolved timeout.
- [ ] On error, the toast updates in place to the error options with
      `toast_type = Error` and gains a dismiss timer per the resolved timeout.
- [ ] Loading/success/error options accept either plain text or full update options;
      success/error variants may compute options from the resolved value/error
      (Rust closures replacing `resolvePromiseOptions`).
- [ ] A promise resolution arriving after the toast was closed/ending does not revive
      or block dismissal (ending-update-ignored rule).
- [ ] Future execution uses GPUI executors (`cx.spawn` /
      `cx.background_executor()`); no JS Promise semantics are ported.
- [ ] `ToastManager::promise` works from outside the provider subtree once bound.

### Focus and keyboard behavior

- [ ] F6 (via `actions.rs` global/window key dispatch) focuses the viewport when
...
- [ ] Shift+Tab on the focused viewport returns focus to the recorded previous focus
...
- [ ] Focus-driven expansion only occurs for keyboard (focus-visible-like) focus where
...
- [ ] `ToastContent<P>` wraps toast contents, watches its own size, and triggers
      height recalculation.
- [ ] `ToastTitle<P>` renders the toast record's title by default (children override);
      renders nothing when no title exists.
- [ ] `ToastDescription<P>` renders the toast record's description by default
      (children override); renders nothing when no description exists.
- [ ] `ToastClose<P>` closes its own toast on click/keyboard activation.
- [ ] `ToastAction<P>` renders the typed action (from the toast record or its own
      children), invokes the action callback on activation, and renders nothing when
      no action content exists.
- [ ] Title/Description/Close/Action expose `toast_type` in their style states;
      Close/Action are focusable GPUI interactive elements.
- [ ] DOM id wiring (`setTitleId`/`setDescriptionId` for `aria-labelledby`/
      `aria-describedby`) is not ported; it returns with the AccessKit follow-up.

### Styling/state exposure

- [ ] One `*StyleState` struct per part that draws, in `style_state.rs`:
      ...
      and gets none).
- [ ] All drawing parts expose `style_with_state(...)`.
- [ ] Every Base UI data attribute / CSS var fact is available as typed style state:
...
- [ ] No `className`, web `style`, render-prop, CSS-var, or DOM data-attribute public
      API is introduced.
...
- [ ] Newest-first ordering and metadata (dom index, visible index, offset_y) stay
      synchronized across add/close/remove/height mutations.
- [ ] Default timeout 5000 ms / per-toast override / `0` sticky / `Loading` no-timer.
- [ ] Upsert-by-id resets the timer, increments update generation, keeps queue
      position; upsert onto an ending toast removes silently and re-adds.
- [ ] Update on ending toast is ignored; update timeout changes reschedule; update to
      `Loading` clears the timer; update from `Loading` schedules one.
- [ ] Limit: newest 3 visible, older flagged limited; limit change recomputes flags;
      ending toasts excluded from the count.
- [ ] Pause records remaining ms; resume restarts with remaining; pause/resume are
      idempotent; toast added while expanded/out-of-focus starts paused.
- [ ] Re-pause regression suite mirroring `store.test.ts` timer-pausing cases.
- [ ] Stale timer generations are no-ops after reset/pause/close.
- [ ] `close(None)` closes all, clears timers, fires each `on_close` once;
      `on_remove` fires on removal, not on close; skip-on-remove path for
      upsert-over-ending.
- [ ] Hover/focus flags reset when no active toasts remain.
- [ ] Swipe state machine: direction locking, damping of non-permitted axes,
...
- [ ] `cargo test -p base_gpui toast` passes.
